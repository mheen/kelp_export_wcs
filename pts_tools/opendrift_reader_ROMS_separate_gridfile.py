...
class Reader(BaseReader, StructuredReader):
    ...
    def __init__(self, filename=None, name=None, gridfile=None):
        ...
        # MvdM: opening gridfile if given
        if gridfile is not None:
            try:
                logger.info(f'Opening gridfile: {gridfile}')
                self.grid_Dataset = xr.open_dataset(gridfile, decode_times=False)
            except Exception as e:
                raise ValueError(e)

        if 'Vtransform' in self.Dataset.variables:
            self.Vtransform = self.Dataset.variables['Vtransform'].data  # scalar
            logger.debug(f'Vtransform = {self.Vtransform}')
        # MvdM: read Vtransform from gridfile
        elif hasattr(self, 'grid_Dataset'):
            if 'Vtransform' in self.grid_Dataset.variables:
                self.Vtransform = self.grid_Dataset.variables['Vtransform'].data
                logger.debug(f'Reading Vtransform from grid file, Vtransform = {self.Vtransform}')
        else:
        ...
        for var in list(self.ROMS_variable_mapping):  # Remove unused variables
            # MvdM: added grid file variables if present
            if hasattr(self, 'grid_Dataset'):
                if var not in self.Dataset.variables and var not in self.grid_Dataset.variables:
        ...
        if 'lat_rho' in self.Dataset.variables:
            ...
            if gridfile is None:
                ...
                # MvdM: removed opening file, now done above if gridfile is given
                self.lat = self.grid_Dataset.variables['lat_rho'][:]
                self.lon = self.grid_Dataset.variables['lon_rho'][:]
                self.lon = self.lon.data # MvdM: extract data (needed because opening with xarray)
                self.lat = self.lat.data
        ...
        for var_name in self.grid_Dataset.variables: # MvdM: added, so that grid file variables are also included
            if var_name in self.ROMS_variable_mapping.keys():
                var = self.grid_Dataset.variables[var_name]
                self.variables.append(self.ROMS_variable_mapping[var_name])
    ...
    def get_variables(self, requested_variables, time=None,
        ...
        if z.min() == 0 or not hasattr(self, 'hc'):
            ...
            if not hasattr(self, 'sea_floor_depth_below_sea_level'):
                if 'h' in self.Dataset.variables: # MvdM: added condition
                    logger.debug('Reading sea floor depth...')
                    self.sea_floor_depth_below_sea_level = \
                        self.Dataset.variables['h'][:]
                elif 'h' in self.grid_Dataset.variables: # MvdM: added
                    logger.debug('Reading sea floor depth from grid file...')
                    self.sea_floor_depth_below_sea_level = \
                        self.grid_Dataset.variables['h'][:]
        ...
        for par in requested_variables:
            ...
            elif varname[0] in self.grid_Dataset.variables: # MvdM: added, what to do "else"?
                var = self.grid_Dataset.variables[varname[0]]

            if par == 'land_binary_mask':
                if not hasattr(self, 'land_binary_mask'):
                    if 'mask_rho' in self.Dataset.variables: # MvdM: added condition, in case mask only in gridfile
                        # Read landmask for whole domain, for later re-use
                        self.land_binary_mask = \
                            1 - self.Dataset.variables['mask_rho'][:]
                    else: # MvdM: added
                        if 'mask_rho' in self.grid_Dataset.variables:
                            self.land_binary_mask = \
                                1 - self.grid_Dataset.variables['mask_rho'][:]
            ...
            if par not in mask_values:
                ...
                if par == 'x_sea_water_velocity':
                    if not hasattr(self, 'mask_u'):
                        ...
                        elif 'mask_u' in self.grid_Dataset.variables: # MvdM: added
                            self.mask_u = self.grid_Dataset.variables['mask_u'][:]
                        elif 'mask_rho' in self.Dataset.variables:
                            self.mask_u = self.Dataset.variables['mask_rho'][:]
                        elif 'mask_rho' in self.grid_Dataset.variables: # MvdM: added
                            self.mask_u = self.grid_Dataset.variables['mask_rho'][:]
                        else:
                            continue
                    ...
                    if not hasattr(self, 'mask_v'):
                        ...
                        elif 'mask_v' in self.grid_Dataset.variables: # MvdM: added
                            self.mask_v = self.grid_Dataset.variables['mask_v'][:]
                        elif 'mask_rho' in self.Dataset.variables:
                            self.mask_v = self.Dataset.variables['mask_rho'][:]
                        elif 'mask_rho' in self.grid_Dataset.variables: # MvdM: added
                            self.mask_v = self.grid_Dataset.variables['mask_rho'][:]
                        else:
                            continue
                    ...
                    if not hasattr(self, 'mask_rho'):
                        ...
                        if 'mask_rho' in self.Dataset.variables: # MvdM: added condition
                            self.mask_rho = self.Dataset.variables['mask_rho'][:]
                        elif 'mask_rho' in self.grid_Dataset.variables: # MvdM: added, what to do if this is False?
                            self.mask_rho = self.grid_Dataset.variables['mask_rho'][:]
        ...
        if 'x_sea_water_velocity' or 'sea_ice_x_velocity' \
            ...
            if not hasattr(self, 'angle_xi_east'):
                if 'angle' in self.Dataset.variables: # what happens when this is not true?
                    logger.debug('Reading angle between xi and east...')
                    self.angle_xi_east = self.Dataset.variables['angle'][:]
                else: # MvdM added
                    if 'angle' in self.grid_Dataset.variables: # what if this is not true?
                        logger.debug('Reading angle between xi and east from gridfile...')
                        self.angle_xi_east = self.grid_Dataset.variables['angle'][:]
